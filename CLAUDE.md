...
**IMPORTANT**: All simulation results must be saved in the `results/` directory. This includes:
- Local simulation outputs
- Downloaded results from Kubernetes clusters
- Calibration results
- Analysis outputs

The structure is: `results/{config_name}/{run_id}/`

**DO NOT** create `downloaded_results/` folders - all results go in `results/`!

## Code style rules
- Always put imports at the top of files
- Never add shebang lines (#!/usr/bin/env python3) to Python scripts 
